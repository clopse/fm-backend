    ...
    try:
        ...
        for attempt in range(12):  # 2 minutes max
            ...
            if status == "completed":
                ...
                print(f"📋 Standardization result status: {result_json.get('status')}")
                ...
                if not parsed:
                    ...
                    if not parsed:
                        print(f"❌ Empty result in both 'result' and 'data' fields")
                        print(f"❌ Full response: {result_json}")
                        return
                
                print(f"✅ Got parsed data with keys: {list(parsed.keys()) if parsed else 'None'}")
